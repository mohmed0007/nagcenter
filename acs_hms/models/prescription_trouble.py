...
class ACSPrescriptiontroubleOrder(models.Model):
    _name='prescription.trouble.order'
    _description = "Prescription Trouble Order"
    ...
    diseases_ids = fields.Many2many('hms.diseases', 'diseases_prescription_trouble_rel', 'diseas_trouble_id', 'prescription_trouble_id', 
    ...
    prescription_line_ids = fields.One2many('prescription.trouble.line', 'prescription_id', string='Prescription line')
    ...
    appointment_trouble_id = fields.Many2one('hms.appointment', ondelete="restrict", 
    ...
    medical_alert_ids = fields.Many2many('acs.medical.alert', 'prescription_trouble_medical_alert_rel','prescription_trouble_id', 'alert_trouble_id',
        string='Medical Alerts', related="patient_id.medical_alert_ids")
    alert_count = fields.Integer(compute='_get_alert_count', default=0)
    old_prescription_id = fields.Many2one('prescription.trouble.order', 'Old Prescription', copy=False, states=READONLY_STATES)
    acs_kit_id = fields.Many2one('acs.product.kit', string='Kit', states=READONLY_STATES)

    @api.model
    def create(self, values):
        res = super(ACSPrescriptiontroubleOrder, self).create(values)
    ...
    def on_change_group_id(self):
        product_lines = []
        for rec in self:
            appointment_trouble_id= rec.appointment_trouble_id and rec.appointment_trouble_id.id or False
            for line in rec.group_id.medicament_group_line_ids:
                product_lines.append((0,0,{
                    ...
                    'appointment_id': appointment_trouble_id,
                }))
            rec.prescription_line_ids = product_lines

    @api.onchange('appointment_trouble_id')
    def onchange_appointment(self):
        if self.appointment_trouble_id and self.appointment_trouble_id.treatment_id:
            self.treatment_id = self.appointment_trouble_id.treatment_id.id

    def unlink(self):
        for rec in self:
            if rec.state not in ['draft']:
                raise UserError(_('Prescription Order can be delete only in Draft state.'))
        return super(ACSPrescriptiontroubleOrder, self).unlink()
    ...
    def button_confirm(self):
        for app in self:
            ...
            if not app.name:
                app.name = self.env['ir.sequence'].next_by_code('prescription.trouble.order') or '/'

    def print_report(self):
        return self.env.ref('acs_hms.report_hms_prescription_trouble_id').report_action(self)
...
class ACSPrescriptionTroubleLine(models.Model):
    _name = 'prescription.trouble.line'
    ...
    prescription_id = fields.Many2one('prescription.trouble.order', ondelete="cascade", string='Prescription')
    ...
    active_component_ids = fields.Many2many('active.comp','product_trouble_pres_comp_rel','product_trouble_id','pres_trouble_id','Active Component')
    ...
    days = fields.Float("How Many Days",default=1.0)
