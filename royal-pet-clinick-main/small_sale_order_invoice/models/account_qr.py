...
class AccountMove(models.Model):
    ...
    # item_len  = fields.Integer(compute="change_size_page",store=True)
    ...
    # @api.onchange('item_len')
    def change_size_page(self,item):
        for rec in self:
            print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
            paper_format = self.env['report.paperformat'].sudo().search([('name', '=', 'invoice small Format')])
            # rec.item_len =  len(rec.invoice_line_ids)
            if  item > 3:
                paper_format.page_height = 200 + (item * 15)
            else:
                paper_format.page_height = 195
