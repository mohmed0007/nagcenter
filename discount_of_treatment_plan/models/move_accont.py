...
class account_move(models.Model):
			...
			for rec in self:
				if res_config.tax_discount_policy_prescribtion_order == 'tax':
					...
					if rec.discount_type == 'line':
						...
						print("in else 2",rec.discount_method)
						rec.amount_total = rec.amount_tax + rec.amount_untaxed - rec.discount_amt
	...
	def create(self, vals_list):
		...
		tax_discount_policy_prescribtion_order = settings.tax_discount_policy_prescribtion_order
		if not tax_discount_policy_prescribtion_order and res.discount_amt == 0:
			return res
		if len(vals_list):
