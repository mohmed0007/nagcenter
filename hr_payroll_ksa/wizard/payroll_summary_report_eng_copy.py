...
class PayrollSummaryReport(models.TransientModel):
    ...
    def print_reports(self):
        ...
        print ("BAtchids::::", self.batch_ids, self.status)
        ...
        print ("payslip_domain", payslip_domain)
        ...
        worksheet.merge_range(0, 1, 1, 1, '', detail_head)# self.env.company.logo
        ...
        if self.env.user.company_id.logo:
            ...
            worksheet.insert_image('B1:B2', '/tmp/image.jpg')# {'x_scale': 0.5, 'y_scale': 0.5}
        
        worksheet.merge_range(0, 2, 0, 14, self.env.company.name, detail_head)

        worksheet.set_row(1, 35)
        #worksheet.write(1, 1, 'LOGO', detail_head)
        worksheet.merge_range(1, 2, 1, 14, self.file_name, detail_head)

        worksheet.merge_range('C4:E4', 'Benefits', merge_head)
        worksheet.merge_range('J4:L4', 'Deductions', merge_head)

        # Header
        worksheet.set_row(4, 30)
        worksheet.write(4, 0, '', bank_head)
        worksheet.write(4, 1, 'Employee', bank_head)
        worksheet.write(4, 2, 'Basic Salary', bank_head)
        worksheet.write(4, 3, 'Housing Allowance', bank_head)
        worksheet.write(4, 4, 'Transportation Allowance', bank_head)
        worksheet.write(4, 5, 'Allowance Management', bank_head)
        worksheet.write(4, 6, 'Other Allowances', bank_head)
        worksheet.write(4, 7, 'Additional', bank_head)
        worksheet.write(4, 8, 'Total Benefits', bank_head)
        worksheet.write(4, 9, 'Advance Payment', bank_head)
        worksheet.write(4, 10, 'Discount', bank_head)
        worksheet.write(4, 11, 'Social Insurance', bank_head)
        worksheet.write(4, 12, 'Total Deductions', bank_head)
        worksheet.write(4, 13, 'The Net Salary', bank_head)
        worksheet.write(4, 14, 'Notes', bank_head)
        
        department_ids = self.env['hr.department'].search([])
        branch_ids = self.env['hr.branch'].search([])

        print (":::::", department_ids, branch_ids, payslip_ids)
        all_branch_data = {}
        ...
        for department in department_ids:
            ...
            print ("payslip_list:::", department.name, payslip_list_ids)
        ...
        row += 1
        worksheet.merge_range(row, 0, row, 1, 'All Branch', bank_head)
        ...
        row += 2
        # Brach Header:
        worksheet.merge_range(row, 2, row, 4, 'Benefits', merge_head)
        worksheet.merge_range(row, 9, row, 11, 'Deductions', merge_head)
        
        row+=1
        worksheet.set_row(row, 30)
        worksheet.write(row, 0, '', bank_head)
        worksheet.write(row, 1, 'Employee', bank_head)
        worksheet.write(row, 2, 'Basic Salary', bank_head)
        worksheet.write(row, 3, 'Housing Allowance', bank_head)
        worksheet.write(row, 4, 'Transportation Allowance', bank_head)
        worksheet.write(row, 5, 'Allowance Management', bank_head)
        worksheet.write(row, 6, 'Other Allowances', bank_head)
        worksheet.write(row, 7, 'Additional', bank_head)
        worksheet.write(row, 8, 'Total Benefits', bank_head)
        worksheet.write(row, 9, 'Advance Payment', bank_head)
        worksheet.write(row, 10, 'Discount', bank_head)
        worksheet.write(row, 11, 'Social Insurance', bank_head)
        worksheet.write(row, 12, 'Total Deductions', bank_head)
        worksheet.write(row, 13, 'The Net Salary', bank_head)
        worksheet.write(row, 14, 'Notes', bank_head)
        ...
        worksheet.write(row, 1, 'Total Salaries', merge_head)
        ...
        row += 2
        worksheet.write(row, 1, '‫Total', bank_head)# ar_001 ar_SY
        ...
        row += 2
        worksheet.write(row, 1, 'Human Resources')
        worksheet.write(row, 4, 'Accountant')
        worksheet.write(row, 8, 'Financial Manager')
        worksheet.write(row, 12, 'Chief Executive Officer')
